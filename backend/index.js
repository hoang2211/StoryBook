...
app.post("/add-travel-story", autheticateToken, async(req,res) =>{
   ...
   //Converr vistedDAte from mili second to date object
...
app.get("/get-all-travel-story", autheticateToken, async(req,res) =>{
    const { userId } = req.user;
...
app.post("/image-upload",upload.single("image"), async(req,res) => {
    try{
        ...
        const imageUrl = `http://localhost:4560/uploads/${req.file.filename}`;
...
app.delete("/delete-image", async (req,res) =>{
    ...
    try{
        ...
        if (fs.existsSync(filePath)){
            ...
            res.status(200).json({message:"Images deleted succesfully"});
        } else{
...
app.put("/edit-story/:id",autheticateToken, async(req,res) =>{
    ...
    try{
        ...
        const placeholderImgUrl = `http://localhost:4560/assets/placeholder.jpg`;
        ...
        await travelStory.save();
        res.status(200).json({story: travelStory, message:"Update Succesfull"});
    }catch(error){
        res.status(500).json({error:true, message:error.message});
    }
})
...
app.delete("/delete-story/:id",autheticateToken, async(req,res) => {
    ...
    try{
         ...
         fs.unlink(filePath, (err)=>{
            if(err){
                console.err("Failed to delete image file:", err);
            }
         });
         res.status(200).json({message:"Travel Story delete successfully"});
    }
    catch(err){
        res.status(500).json({error:true, messagge:err.message});
    }    
});

//Update isFavourite
app.put("/update-is-favourtite/:id", autheticateToken, async(req,res)=>{
    const {id} = req.params;
